class WeatherWidget extends HTMLElement {
  ...
  initSettings() {
    ...
    this.useFahrenheit = localStorage.getItem("tempUnit") === "f";
    this.useMph = localStorage.getItem("windUnit") === "mph";
    this.use24h = localStorage.getItem("timeFormat") === "24h";
    this.useInches = localStorage.getItem("precipUnit") === "in";
  }
  ...
  <div class="widget">
    ...
    <div class="switch-row">
      <div class="switch-group">
        <input type="radio" name="tempUnit" id="temp-c" value="c" checked>
        <label for="temp-c">C</label>

        <input type="radio" name="tempUnit" id="temp-f" value="f">
        <label for="temp-f">F</label>
      </div>

      <div class="switch-group">
        <input type="radio" name="windUnit" id="wind-kmh" value="kmh" checked>
        <label for="wind-kmh">km/h</label>

        <input type="radio" name="windUnit" id="wind-mph" value="mph">
        <label for="wind-mph">mph</label>
      </div>

      <div class="switch-group">
        <input type="radio" name="timeFormat" id="time-12" value="12h" checked>
        <label for="time-12">12</label>

        <input type="radio" name="timeFormat" id="time-24" value="24h">
        <label for="time-24">24</label>
      </div>

      <div class="switch-group">
        <input type="radio" name="precipUnit" id="precip-mm" value="mm" checked>
        <label for="precip-mm">mm</label>

        <input type="radio" name="precipUnit" id="precip-in" value="in">
        <label for="precip-in">in</label>
      </div>
    </div>
...
`;
  ...
  setupUnitSwitches() {
    const bindGroup = (name, storageKey, onChange) => {
      const inputs = this.shadowRoot.querySelectorAll(`input[name="${name}"]`);
      const storedValue = localStorage.getItem(storageKey);

      if (storedValue) {
        const storedInput = this.shadowRoot.querySelector(
          `input[name="${name}"][value="${storedValue}"]`
        );
    ...
    bindGroup("tempUnit", "tempUnit", value => {
      this.useFahrenheit = value === "f";
      this.renderAll();
    });

    bindGroup("windUnit", "windUnit", value => {
      this.useMph = value === "mph";
      this.renderAll();
    });

    bindGroup("timeFormat", "timeFormat", value => {
      this.use24h = value === "24h";
      this.renderAll();
    });

    bindGroup("precipUnit", "precipUnit", value => {
  ...
  async getField(field) {
    const response = await fetch(
      `http://frogcastproxy.onrender.com/forecast?latitude=${this.latitude}&longitude=${this.longitude}&fields=${field}`
    );
