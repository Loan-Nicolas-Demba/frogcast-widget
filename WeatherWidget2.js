...
`;
  ...
  setupEvents() {
    ...
    this.shadowRoot.addEventListener("click", (e) => {
      ...
      this.renderHourlyDetails(index);
    });
  }
  ...
  async loadForecast() {
    ...
    try {
      ...
      this.tempData = await this.getField("t2m");
  ...
  async renderForecast() {
    ...
    container.innerHTML = days.map((day, index) => `
      <article class="forecast__day" data-day-index="${index}">
  ...
  renderHourlyDetails(dayIndex = 0) {
    ...
    if (!container || !this.tempData) return;

    const days = this.groupHourlyByDay(this.tempData, this.dayAmount);
    ...
    container.innerHTML = `
      ${day.hours.map(hour => `
        <article class="hourly__item">
          ...
          <div class="hourly__meta">
            <span class="hourly__time">${hour.time}</span>
            <span class="hourly__temp">${Math.round(hour.value)}°</span>
          </div>
        </article>
      `).join("")}
    `;
