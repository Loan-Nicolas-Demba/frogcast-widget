    ...
    <div class="current">
      ...
      <div class="temperature-row">
        ...
        <div class="weather-details">
          ...
          <p id="felt-temperature">Real Feel: 18°C</p>
        </div>
      </div>
...
`;
  ...
  renderHourlyDetails(dayIndex = 0) {
    ...
    container.innerHTML = day.hours.map((hour, index) => `
      <div class="hourly__item">
        ...
        <p class="hourly__temp">${Math.round(hour.t2m)}°C</p>

        <div class="hourly__divider"></div>

        <div class="hourly__details">
          <p class="hourly__precipitation">${Math.round(this.msToKmh(hour.mtpr) * 10) / 10 ?? 1110}mm</p>
          <p class="hourly__wind-direction">${this.degreesToCardinal(hour.windDirection)}</p>
          <p class="hourly__wind-gust">${Math.round(this.msToKmh(hour.windGust) * 10) / 10  ?? "--"}km/h</p>
          <p class="hourly__wind-speed">${Math.round(this.msToKmh(hour.windSpeed) * 10) / 10 ?? "--"}km/h</p>
  ...
  formatTemperature(celsius) {
    ...
    return this.useFahrenheit
      ? `${Math.round((celsius * 9 / 5 + 32) * 10) / 10}°F`
  ...
  getTemperature() {
    ...
    this.shadowRoot.getElementById("temperature").textContent = `${Math.round(temp * 10) / 10}°C`;
    return temp;
  }
